...
--                Code cleanup using ZBS evaluate

--[[
  R client software with Lua representation of some R objects. 
  Sample usage:
...
-------------------------------------
local function buildstrmsg(rexp, command)
  -- QAP1 message header
  command = command or 3      -- command specifies the request or response type
  local length = (#rexp + 4)  -- length of the message (bits 0-31)
  ...
  return vstruct.write("4*u4 u1 u3 s" .. #rexp, data)
end

-------------------------------------
-- Build an SEXP string message
-- @param data (matrix, frame, factor)
-------------------------------------
--local function buildStrSEXP(str)
--  local dptype = 4            -- QAP1_DATATYPES[4] = "DT_STRING"
--  local data = {dptype, #str, str}
--  return vstruct.write("u1 u3 s" .. #str, data)
--end

--local function buildArrayIntSEXP(array)
--  local dptype = 10
--  local data = array
--  table.insert(data, dptype, 1)
--  table.insert(data, #array, 1)
--  return vstruct.write("u1 u3 " .. #array .. "*i4", data)
--end

--local function buildArrayDoubleSEXP(array)
--  local dptype = 11
--  local data = array
--  table.insert(data, dptype, 1)
--  table.insert(data, #array, 1)
--  return vstruct.write("u1 u3 " .. #array .. "*f8", data)
--end
...
function rclient._calltcp(rsserver, rsport, msg, read)
  ...
  if read and status~='timeout' then
    ...
    if string.len(s2)==16 then
      local qmsgheader = vstruct.read("4*u4", s2)
...
function rclient.evaluate(rexp, rsserver, rsport, command)
  ...
  local qmsgheader = vstruct.read("4*u4", string.sub(qmsg, 1, 16))
...
function rclient.evaluateVoid(rexp, rsserver, rsport)
  return rclient.evaluate(rexp, rsserver, rsport, 2)
end
...
function rclient._set(t, key, value)
  if value.toR then
    ...
    return rclient.evaluate(vector:new(value):toR(key), server.host, server.port, 2)
  end
end
